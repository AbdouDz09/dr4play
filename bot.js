...
client.on('message', message => {
	...
	.setFooter('============================================')
        .addField('${prefix}ghelp', 'اظهاراوامر العامة')
...
client.on('message', message => {
    if (message.content === '${prefix}ghelp') {
        let helpEmbed = new Discord.RichEmbed()
        .setTitle('**أوامر عامة...**')
        .addField('${prefix}avatar', "افاتار الشخص المطلوب")
        .addField('${prefix}gif', 'البحث عن جيف انت تطلبه')
        .addField('${prefix}ping', 'معرفة ping البوت')
        .setFooter('by : AbdouFERSAOUI_Dz 👻็็็็็็็็็#5217')
      message.channel.send(helpEmbed);
    }
});
...
client.on('message', message => {//Toxic Codes
  ...
  if (message.content.startsWith(adminprefix + 'sets')) {
    client.user.setGame(argresult, "https://www.twitch.tv/abdoufersaoui_");
      message.channel.send(`**✅**`)//Toxic Codes
  }
  if (message.content.startsWith(adminprefix + 'zetname')) {
  client.user.setUsername(argresult).then
      message.channel.send(`Changing The Name To ..**${argresult}** `)
} else
if (message.content.startsWith(adminprefix + 'zetavatar')) {//Toxic Codes
